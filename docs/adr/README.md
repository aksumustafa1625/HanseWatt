...
**alternatives considered**. ADRs are version-controlled next to the code so the _reasoning_
behind past decisions stays discoverable — not just the _shape_ of the result.
...
4. **Continuity** — the build is phased over many sessions; ADRs are the durable memory of
   _why_, complementing `CLAUDE.md` (the _how/where_ session guide).
