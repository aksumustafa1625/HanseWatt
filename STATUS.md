...
| Grounded actions (8) | `HWIdentifyCustomerAction` (**two-factor**: email + Kundennummer → Account) · `HWGetLatestBillAction` · `HWExplainConsumptionAction` (+64.6 %) · `HWCreateCaseAction` · `HWAnswerFromKnowledgeAction` (cited help article) · `HWProposeTariffChangeAction` · `HWConfirmTariffChangeAction` · **`HWRegisterMoveAction`** | `force-app-actions/main/default/classes/` |
| Service layer (8) | `HWCustomerService` · `HWBillingService` · `HWConsumptionService` · `HWCaseService` · `HWKnowledgeService` · `HWTariffService` · **`HWMoveService`** · **`HWIds`** (the LLM→Apex trust boundary) — `with sharing`, `WITH USER_MODE`, bulk-safe | `force-app-services/main/default/classes/` |
| **Identity is a token (ADR-021)** | An email alone releases **nothing**. `HWCustomerService.verify()` requires the email **and** the **Kundennummer** printed on the bill (`Account.Customer_Number__c`) — how a German utility hotline actually authenticates. To see Johann Huber's data you need **his** customer number: the neighbour asking doesn't have it, and the model can't invent it (compared server-side, never echoed in any output).<br><br>A failed attempt reveals **nothing** — not even whether the email is registered, so the agent is not an account-enumeration oracle. The three out-of-the-box Salesforce actions that could resolve an Account from an email alone (`Identify Customer By Email`, `Update Verified Contact`, `Get All Cases For Contact`) were **removed** from the agent: a guarantee is only as strong as the weakest path the planner may take.<br><br>**Verified live:** a verified customer asks for another customer's bill → the classifier picks the **right** topic, the agent has **7 actions** available — and calls **none of them**. Salesforce's own Output Evaluation marks the refusal **GROUNDED**. | `docs/demo/images/dsgvo-refusal.png`, ADR-021 |
| **`HWIds` — an action input is untrusted (ADR-021)** | Caught mid-recording: asked for another customer's bill, the planner put the **email address** into the `Account Id` slot. The actions declared it as Apex `Id`, so coercion threw **before a line of our code ran** and the customer saw *"Ein Fehler ist aufgetreten"* — a stack trace where the privacy refusal belonged. **A guardrail that crashes is not a guardrail.**<br><br>An `@InvocableVariable` is not a parameter from a colleague's code; it is **a string a language model chose**. All 8 actions now parse the id through `HWIds`, which maps blank / malformed / wrong-sObject values to `null` — which already means "not identified". **A bad id is a refusal, never an exception.** | `force-app-services/`, ADR-021 |
| **Umzug is filed, not narrated (ADR-021)** | The agent could *explain* a move from a cited article and then had nowhere to put the customer's answer. `HWRegisterMoveAction` files a **Move case** (`HW_Topic__c = 'Move'` — the record type dormant since Faz 1) — but only once the customer supplies the facts **only they** have. *"Ich ziehe **nächsten Monat** um"* is **not a date**, and the agent does not turn it into one: a missing move-out date or final meter reading is a **refusal with zero DML**, because an estimated reading becomes an estimated final bill. **Verified live:** the agent cites the article, asks, and only then files the case with the date, the customer's own reading, and the new address. | `docs/demo/images/umzug-case-1.png`, ADR-021 |
...
| **Identity is two-factor (ADR-021)** | The privacy refusal used to be the *model's good manners*: the topic instruction said "never show another customer's data", but `HW Identify Customer(email)` would happily have returned **any** customer's Account Id, and the bill action would have grounded on it. No Apex refused — the model simply chose not to walk the path. Under pressure in the live transcript, the refusal visibly *degraded* (firm → non-answer). So identity is now **two-factor**: email **+ Kundennummer** (`Account.Customer_Number__c`, e.g. `HW-100001`), matched server-side, both must belong to the **same** account. **The model cannot supply the second factor** — to see Johann Huber's bill you need Johann Huber's customer number, and the person asking does not have it. A failure reveals **nothing**, not even whether the email is registered (no account-enumeration oracle) and not the account name. 11 tests written from the *attacker's* side. | `force-app-services/`, ADR-021 |
...
| Tests | **16 test classes / 115 test methods** — one per service **and** one per action (1:1), all branches + 200-record bulk-safety tests; **97 % org-wide coverage**, every class ≥ 94 %. The tests that matter most assert the **refusals**: the first confirmation changes nothing, an invented / foreign / expired request number is refused with **zero DML**, an email in the Account Id slot does not throw, a move is never filed on an invented date. Every rule is proven **before** the agent runs it — at **zero Flex Credits**. | `force-app-tests/main/default/classes/` |
